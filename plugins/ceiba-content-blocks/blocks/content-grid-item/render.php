...
return function( $attributes = [] ) {
  ...
  <div class="content-grid-item">
    ...
    <div class="content-grid-item__content">
      <?php if ( $heading ) printf( '<%1$s class="content-grid-item__title">%2$s</%1$s>', esc_html( $tag ), wp_kses_post( $heading ) ); ?>
      <?php if ( $text ) echo '<p class="content-grid-item__text">' . wp_kses_post( $text ) . '</p>'; ?>
    </div>
  </div>
  <?php
  return ob_get_clean();
